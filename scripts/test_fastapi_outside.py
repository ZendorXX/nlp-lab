"""Отправляет запросы с хоста в FastAPI-обертку в одиночном и пакетном режимах."""
...
def call_fastapi(prompt: str) -> dict:
    """Вызывает FastAPI-эндпоинт и возвращает распарсенный JSON-ответ."""
...
def print_result(prompt: str, data: dict) -> None:
    """Печатает отправленный запрос и ответ FastAPI в удобном формате."""
...
def run_single(prompt: str) -> None:
    """Отправляет один пользовательский промпт в FastAPI и печатает результат."""
    data = call_fastapi(prompt)
    print_result(prompt, data)


def run_batch() -> None:
    """Отправляет 10 предустановленных промптов через FastAPI и печатает результаты."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Отправка пользовательских и предустановленных запросов в FastAPI-обертку в Docker."
    )
    parser.add_argument(
        "--prompt",
        type=str,
...
def main() -> None:
    ...
    prompt = args.prompt
    if not prompt:
        prompt = input("Введите сообщение для отправки в FastAPI: ").strip()
    if not prompt:
